...
from assign_bit import BitwidthAllocatorMBNetV2
...
from model.MobileNetV2_qt1 import *
from model.MobileNetV2_qt2 import *
...
def get_args():
    ...
    parser.add_argument('--arch', type=str, default='mobilenetv2', choices=('resnet_56','vgg_16_bn','mobilenetv2'), help='The architecture to prune and the resulting model and logs will use this') 
    parser.add_argument('--resume', type=str, default='./ckpt/MobileNetV2_CIFAR100.t7',help='load the model from the specified checkpoint')
    ...
    parser.add_argument("--pruner", type=str, default='FilterPrunerMBNetV2',choices=('FilterPrunerResNet','FilterPrunerVGG', 'FilterPrunerMBNetV2'),help='Different network require differnt pruner implementation')
    parser.add_argument("--Bops_counter", type=str, default='BOPsCounterMBNetV2',choices=('BOPsCounterResNet','BOPsCounterVGG','BOPsCounterMBNetV2'),help='Different network require differnt Bops counter implementation')
    parser.add_argument("--rankPath", type=str, default='./rank_conv/CIFAR100/mobilenetV2', help='The path of ranks of convolution layers')
    ...
    parser.add_argument("--lub", type=str, default='./log_50/mobilenetv2_CIFAR100_ea_min.data', help='The affine transformations')
    parser.add_argument("--savename", type=str, default='mobilenetv2_CIFAR100')
    # parser.add_argument("--ori_channel", type=str, default='[32]*2+[16]+[96]*2+[24]+[144]*2+[24]+[144]*2+[32]+[192]*2+[32]+[192]*2+[32]+[192]*2+[64]+[384]*2+[64]+[384]*2+[64]+[384]*2+[64]+[384]*2+[96]+[576]*2+[96]+[576]*2+[96]+[576]*2+[160]+[960]*2+[160]+[960]*2+[160]+[960]*2+[320]+[1280]', help='The original_channel of different network')
    ...
    parser.add_argument("--long_ft", type=int, default=200, help='It specifies how many epochs to fine-tune the network once the pruning is done')
    ...
    parser.add_argument("--qbw_upper",type=int, default='[32] + [6]*17 + [4]*30 + [3]*3 + [32]', help='The upper bound of quantization bid-width for different network')
    ...
    parser.add_argument("--batch_size", type=int, default=64, choices=(16, 32, 64, 128), help='Batch size for training.')
...
class PICPQ:
    ...
    def learn_ranking_ea(self, name, model_desc, tau_hat, long_ft, target):
        ...
        for i in range(GENERATIONS):
            ...
            model = MobileNetV2_CIFAR100(n_class=100).cuda()
    ...
    def model_compress(self, name, long_ft, target=-1, arch =''):
        ...
        ori_model = MobileNetV2_CIFAR100(n_class=100).cuda()
        ...
        ori_statistics = BitwidthAllocatorMBNetV2(ori_model)
        slim_statistics = BitwidthAllocatorMBNetV2(slim_model)
        ...
        model_qt1 = QMobileNetV2_A(n_class = 100, filters_left=sparse_channel, bit=bit).cuda()
        ...
        model_qt2 = QMobileNetV2_A_W(n_class = 100, filters_left=sparse_channel, bit=bit).cuda()
        ...
        bops_counter = eval('BOPsCounterMBNetV2')(self.model, self.rank_type, num_cls=100, rankPath=args.rankPath, device=self.device)
        _, cur_bops = bops_counter.forward(torch.zeros((1,3,32, 32), device=self.device), sparse_channel, bit_conv, bit_conv)
...
if __name__ == "__main__":
    ...
    # model
    basemodel = MobileNetV2_CIFAR100(n_class=100).cuda()
    ...
    filter_pruner = eval('FilterPrunerMBNetV2')(basemodel, 'Rank', num_cls=10, rankPath=args.rankPath, device=device)
