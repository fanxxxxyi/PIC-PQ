...
class BOPsCounterResNet(FilterPruner):
    def trace_layer(self, layer, x, sparse_channel, bit_w, bit_a):
        ...
        if isinstance(layer, nn.Conv2d):
            ...
            h = y.shape[2]
            w = y.shape[3]
            self.cur_bops += h * w * self.conv_in_channels * self.conv_out_channels * layer.weight.size(2) * layer.weight.size(3) * bit_w[self.activation_index] * bit_a[self.activation_index]
                ...
                if is_leaf(child):
                    def new_forward(m):
                        def lambda_forward(x):
                            return self.trace_layer(m, x, sparse_channel, bit_w, bit_a)
        ...
        for layer, method in enumerate(model.features.children()):
            ...
            if isinstance(method, nn.Conv2d):
                if self.activation_index == 0:
                    ...
                    self.cur_bops +=  h * w * self.conv_in_channels * self.conv_out_channels * method.weight.size(2) * method.weight.size(3) * bit_w[self.activation_index] * bit_a[self.activation_index]
...
if __name__ == "__main__":
    ...
    # bops_counter = eval('BOPsCounterResNet')(model, 'Rank', num_cls=10, rankPath='./rank_conv/CIFAR10/resnet_56', device=device)
    # bops_counter = eval('BOPsCounterMBNetV2')(model, 'Rank', num_cls=10, rankPath='./rank_conv/CIFAR100/mobilenetV2', device=device)
    _, cur_bops = bops_counter.forward(torch.zeros((1,3,32, 32), device=device), sparse_channel, bit_w, bit_a)
